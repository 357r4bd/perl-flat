...
for (qw[ abcg abc abdeg abde ab abg abfffg abff ]) {
    ok(/$p/, "as_perl_regex (positives)");

    ok($reg->contains($_), "contains (positives)");
}

for (qw[ aabcg ac abcdeg abdef abffgg ]) {
    ok(!/$p/, "as_perl_regex (negatives)");

    ok(!$reg->contains($_), "contains (negatives)");
}
